...
def check_completion(user_in):
    ...
    try:
        ...
        f = open(f"./Completed/{paper}.txt", 'w')
        f.write('') 
        f.close()
        outcome = f"No questions have previously been attempted from {paper}."
...
#Completed folder stores text files with list of all completed questions - if not present when packaged, creates folder
if "Completed" not in os.listdir(os.getcwd()):
    os.mkdir("./Completed")
else:
    pass     
